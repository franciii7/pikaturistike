...
<?php if(config_item('tree_field_enabled') === TRUE && config_db_item('enable_county_affiliate_roles') === false): ?>
...
$(function() {
        $(".search-form .TREE-GENERATOR select").change(function(){
            ...
            .find('select').each(function(index){
                ...
                if(index > s_level)
                {
                    ...
                    $(this).val('');
                    $("#input_municipality_id").val('');
                }
                else
                {
                    if($(this).find("option:selected").val())
                    $("#input_municipality_id").val($(this).find("option:selected").val());
    ...
    function load_by_field(field_element, autoselect_next, s_values_splited)
    {
        ...
        var s_name_splited = field_element.attr('id').split("_"); console.log(s_name_splited)
        var s_level = parseInt(s_name_splited[4]);
        var s_lang_id = s_name_splited[1];
        var s_field_id = s_name_splited[2];
        // console.log(s_value); console.log(s_level); console.log(s_field_id);
        // Load values for next select
        var ajax_indicator = field_element.parent().parent().parent().find('.ajax_loading');
        var select_element = $("select[id=sinputOption_"+s_lang_id+"_"+s_field_id+"_level_"+parseInt(s_level+1)+"]");
